...
export default function ReceiptVoucherForm({ identity, onSaved, onCancel, onPreviewVoucher, voucherToEdit }: ReceiptVoucherFormProps) {
  // Pull drop options
  ...
  useEffect(() => {
    ...
    if (voucherToEdit) {
      ...
      setPayer(voucherToEdit.payerOrBeneficiary);
      ...
      <div className="border-b border-gray-100 dark:border-zinc-800 pb-4 flex justify-between items-center flex-row-reverse">
        <div className="flex items-center gap-2 flex-row-reverse text-right">
          ...
          <h2 className="text-lg font-black text-gray-900 dark:text-white">
            {voucherToEdit ? `تعديل ${identity.receiptTerm}` : `إصدار ${identity.receiptTerm} جديدة`}
          </h2>
        </div>
